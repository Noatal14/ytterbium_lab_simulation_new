"""Submit the focused five-beam lower-green intensity screen to Zeus."""
...
ROOT = Path("data/validation/mot_3d/five_beam_balance_screen/focused_600")
...
def submit(work_dir):
    ...
    array_file = _write(
        work_dir / "five_beam_balance_array.pbs",
        _header(
            "mot3d_five_beam_balance",
            array=True,
            walltime=settings["pbs_walltime"],
            ncpus=settings["pbs_ncpus_per_shard"],
            mem=settings["pbs_memory_per_shard"],
        )
        + f"""python -u -m studies.scan_3d_mot_five_beam_balance \\
...
""",
    ...
    merge_file = _write(
        work_dir / "five_beam_balance_merge.pbs",
        _header("mot3d_five_beam_balance_merge", walltime="00:15:00", ncpus=1, mem="4gb")
        + f"""python -u -m studies.merge_3d_mot_five_beam_balance \\
    --input-root {ROOT} --output-dir {ROOT}/merged \\
    --graph-dir graphs/mot_3d_five_beam_balance
""",
    )
    merge_job = _submit(merge_file, array_job)
    print(f"Five-beam balance array: {array_job}")
    print(f"Five-beam balance merge: {merge_job}")
    return merge_job
