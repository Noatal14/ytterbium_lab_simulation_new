"""Submit the finite blue-gate geometry comparison to Zeus."""
...
ROOT = Path("data/validation/mot_3d/blue_gate_sequence/focused_600")
...
def submit(work_dir):
    ...
    array_file = _write(
        work_dir / "blue_gate_sequence_array.pbs",
        _header(
            "mot3d_blue_gates",
            array=True,
        ...
        + f"""python -u -m studies.scan_3d_mot_blue_gate_sequence \\
...
""",
    ...
    merge_file = _write(
        work_dir / "blue_gate_sequence_merge.pbs",
        _header("mot3d_blue_gates_merge", walltime="00:15:00", ncpus=1, mem="4gb")
        + f"""python -u -m studies.merge_3d_mot_blue_gate_sequence \\
    --input-root {ROOT} --output-dir {ROOT}/merged \\
    --graph-dir graphs/mot_3d_blue_gate_sequence
""",
    )
    merge_job = _submit(merge_file, array_job)
    print(f"Blue-gate array: {array_job}")
    print(f"Blue-gate merge: {merge_job}")
    return merge_job
