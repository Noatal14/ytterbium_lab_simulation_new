"""Submit polarization-corrected angled-donut optimization to PBS."""
...
def submit_pipeline(work_dir):
    ...
    suffix = "polarization_corrected_900"

    blue_root = DONUT / f"blue_scan_{suffix}"
    blue_array = _write(
        work_dir / "donut_corrected_blue_array.pbs",
        _header("mot3d_donut2_blue", array=True, walltime="05:00:00")
        + f"""python -m studies.scan_3d_mot_blue_slower \\
    --input {INPUT} \\
    --profiles angled_donut \\
    --max-atoms 900 \\
    --num-shards 3 \\
...
""",
    ...
    blue_finish = _write(
        work_dir / "donut_corrected_blue_finish.pbs",
        _header("mot3d_donut2_bfin", walltime="00:30:00", ncpus=1, mem="4gb")
...
""",
    ...
    gradient_root = DONUT / f"gradient_scan_{suffix}"
    gradient_array = _write(
        work_dir / "donut_corrected_gradient_array.pbs",
        _header("mot3d_donut2_grad", array=True, walltime="04:00:00")
...
""",
    ...
    gradient_finish = _write(
        work_dir / "donut_corrected_gradient_finish.pbs",
        _header("mot3d_donut2_gfin", walltime="00:30:00", ncpus=1, mem="4gb")
        + f"""python -m studies.merge_3d_mot_blue_scan_shards \\
...
""",
    ...
    green_root = DONUT / f"green_scan_{suffix}"
    green_array = _write(
        work_dir / "donut_corrected_green_array.pbs",
        _header("mot3d_donut2_green", array=True, walltime="06:00:00")
...
""",
    ...
    green_finish = _write(
        work_dir / "donut_corrected_green_finish.pbs",
        _header("mot3d_donut2_fin", walltime="00:30:00", ncpus=1, mem="4gb")
        + f"""python -m studies.merge_3d_mot_blue_scan_shards \\
...
""",
    ...
    five_selection = (
        FIVE / "green_scan_force_corrected_900/merged/selected_best.json"
    )
    final_job_file = _write(
        work_dir / "donut_corrected_final_summary.pbs",
        _header("mot3d_donut2_sum", walltime="00:15:00", ncpus=1, mem="4gb")
        + f"""python -m studies.summarize_3d_mot_optimization \\
    --selection angled_donut {green_root}/merged/selected_best.json \\
    --selection five_beam_gravity {five_selection} \\
    --output {ROOT}/final_operating_points.json
""",
    )
    final_job = _submit(final_job_file, green_finish_job)
    print(f"Polarization-corrected donut final summary job: {final_job}")
...
def parse_args(argv=None):
    ...
    parser.add_argument(
        "--work-dir",
        default=str(ROOT / "angled_donut_polarization_corrected_pipeline_pbs"),
    )
