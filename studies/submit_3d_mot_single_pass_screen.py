"""Submit the paired 600-atom yz single-pass screen to Zeus."""
...
def submit(work_dir):
    ...
    array_file = _write(
        work_dir / "single_pass_yz_screen_array.pbs",
        _header("mot3d_sp_yz600", True, "04:00:00", 200, "64gb")
        + f"""python -u -m studies.compare_3d_mot_retention \\
    --input {DEFAULT_INPUT} --profiles single_pass \\
    --max-atoms 600 --num-shards 3 --shard-index "$PBS_ARRAY_INDEX" \\
    --npools 200 --t-max 0.1 \\
    --output-dir "{ROOT}/shard_${{PBS_ARRAY_INDEX}}"
""",
    )
    array_job = _submit(array_file)
    merge_file = _write(
        work_dir / "single_pass_yz_screen_merge.pbs",
        _header("mot3d_sp_yz600_merge", False, "00:15:00", 1, "4gb")
        + f"""python -u -m studies.merge_3d_mot_single_pass_screen \\
...
""",
    ...
    print(f"Single-pass yz screen array: {array_job}")
    print(f"Single-pass yz screen merge: {merge_job}")
