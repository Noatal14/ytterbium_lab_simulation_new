"""Submit the focused three-shard single-pass screen to Zeus PBS."""
...
ROOT = Path("data/validation/mot_3d/single_pass_screen/focused_600")
...
def submit(work_dir):
    ...
    array_file = _write(
        work_dir / "single_pass_screen_array.pbs",
        _header(
            "mot3d_single_pass_screen",
            array=True,
            walltime=settings["pbs_walltime"],
            ncpus=settings["pbs_ncpus_per_shard"],
            mem=settings["pbs_memory_per_shard"],
        )
        + f"""python -u -m studies.scan_3d_mot_single_pass_screen \\
...
""",
    ...
    merge_file = _write(
        work_dir / "single_pass_screen_merge.pbs",
        _header("mot3d_single_pass_merge", walltime="00:15:00", ncpus=1, mem="4gb")
        + f"""python -u -m studies.merge_3d_mot_single_pass_screen \\
    --input-root {ROOT} --output-dir {ROOT}/merged \\
    --graph-dir graphs/mot_3d_single_pass_screen
""",
    )
    merge_job = _submit(merge_file, array_job)
    print(f"Single-pass screen array: {array_job}")
    print(f"Single-pass screen merge: {merge_job}")
    return merge_job
