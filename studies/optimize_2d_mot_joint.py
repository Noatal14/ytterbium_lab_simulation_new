...
def evaluate_configuration(
    ...
    for index, ensemble in enumerate(ensembles):
        ...
        if trial is not None:
            trial.report(
                sum(row["conditional_efficiency"] for row in replicates)
                / len(replicates),
                step=index,
            )
            if trial.should_prune():
                raise optuna.TrialPruned()
    return {"replicates": replicates, "statistics": summarize_replicates(replicates)}
...
def optimize_mot(args):
    ...
    study = optuna.create_study(
        ...
        pruner=optuna.pruners.MedianPruner(
            n_startup_trials=min(10, args.n_trials), n_warmup_steps=1
        ),
        load_if_exists=True,
    )
