...
The registered candidate set includes four horizontal (`yz`) crossed-pair
placements, two widened `xz` placements, and one symmetric four-blue-beam
upper-bound design. All use the approved 30-degree angle and the same
force-validated six-beam green MOT; their individual geometry figures are
generated by `python -m graphs_scripts.plot_3d_mot_configurations --config all`.
